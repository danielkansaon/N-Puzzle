...
def retornar_acoes_possiveis(queue, param, excecao):
    ...
    for v in VETOR_MOVIMENTOS_POSSIVEIS:
        vetor_param_split =  list(param_split)
...
def ler_input(caminho):    
        if(os.path.exists(caminho) == False):
            ...
            if(validar_input(line)):
                ...
                print('MSG: O ARQUIVO DE ENTRADA ['+ caminho + '] NAO E VALIDO')     
        return ''
...
def salvar_resultado_out(caminho, resultado):
    try:
        ...
        print('MSG: Houve um erro ao salvar o arquivo [out] no diretorio')   
...
def calcular_passos(raiz):
    ...
    if((raiz == SOLUCAO_NPUZZLE) == False):
        ...
        while (Queue_movimentos_possiveis_vertbrancos and achou_solucao == False): #is not empty
            ...
            if((SOLUCAO_NPUZZLE) in Queue_movimentos_possiveis_vertbrancos): #Verificando se achou a solução
                achou_solucao = True
                break
    else:
        achou_solucao = True
...
def main():           
    ...
    print('QUANTIDADE DE NOS(POSSIBILIDADES) VISITADOS: ' + str(len(movimentos_ja_feitos_vertpretos)))
